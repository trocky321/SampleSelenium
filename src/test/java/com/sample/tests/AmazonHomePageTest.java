...
public class AmazonHomePageTest extends TestBase{
	...
	public void verifyLinksPresentOnHomePage(){
		...
		if(amazonHomePage.getLinksOnHomePage()==0){
			...
			Assert.assertTrue(false,"Links are not Found on the AmazonHomePage!!");
		}
		else
		{
			Assert.assertTrue(true,amazonHomePage.getLinksOnHomePage()+" :Links are Found on the AmazonHomePage!");
